...
void UActionSpreadFire::Action(AGrid* grid, FIntPoint tile, FIntPoint destinyTile)
{
		...
		if (character != nullptr) {
			if (destinyTile != FIntPoint(-1, -1)) {
				if (grid->gridTiles[destinyTile].states.Contains(TileState::isReachable)) {
					...
					if (zombie != nullptr) {
						character->AttackZombieSpreadFire(zombie, destinyTile);
					}
				}
			}
...
bool UActionSpreadFire::NeedLoF()
{
	return true;
}
